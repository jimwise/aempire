...
procedure Main is
   ...
   USAGE : constant String := "[-w water] [-s smooth] [-d delay] [-S save_interval] [-V]";
begin
   ...
   loop
      case GNAT.Command_Line.Getopt(OPTFLAGS) is
         ...
         when 'w' =>
            ...
            if Flag_Small_W < 10 or Flag_Small_W > 90
            then
               Text_IO.Put_Line(Ada.Command_Line.Command_Name & ": -W argument must be in the range 0..90");
         ...
         when 's' =>
            ...
            if Flag_Small_S < 0
            then
               Text_IO.Put_Line(Ada.Command_Line.Command_Name & ": -s argument must be greater than or equal to zero");
               raise Invalid_Command_Line;
            end if;
         ...
         when 'S' =>
           ...
           if Flag_Large_S < 1
           then
              Text_IO.Put_Line(Ada.Command_Line.Command_Name & ": -S argument must be greater than or equal to 1");
              raise Invalid_Command_Line;
           end if;
   ...
   Land := Empire.MAP_SIZE * (100 - Empire.WATER_RATIO) / 100; -- available land
