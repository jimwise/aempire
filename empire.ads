...
package Empire is
   --  these include a `frame' of one cell all around the playable area. XXX
   --  maybe this should change, but it allows easy assumption that we XXX can
   --  go any direction from any playable cell, _then_ check if XXX still on
   --  board.
   ...
   --  these are still from 0 so modulo, etc are easy to use. map location as
   --  index into map
   ...
   package Strings is new Ada.Strings.Bounded.Generic_Bounded_Length
     (STRING_MAX);
   ...
   --  useful constants for accessing sectors should divide evenly into
   --  MAP_HEIGHT, and be likely to fit on screen
   ...
   --  XXX XXX XXX this is really broken. INFINITY, as used here, has to be XXX
   --  XXX XXX small enough that we can add a fair number of them XXX XXX XXX
   --  together within the range of an Integer, yet large enough XXX XXX XXX
   --  that we wouldn't expect to see it. Grrr....
   ...
   --  when adding a new piece type, must add here, in piece_attr below, and at
   --  least also in switch in Empire.Mapping.Vmap_Cont_Scan XXX (undoubtedly
   --  others, too) XXX XXX note that piece types here need to be in preference
   --  order XXX XXX for Find_Obj_At_Loc (least -> most). This can be replaced
   ...
   type Piece_Class_T is (GROUND, AIRCRAFT, SHIP, SPACECRAFT);

   --  in addition to terrain and user types, planning algorithms use following
   --  special symbols in view maps:
   ...
   --  (compmove.c) used two values, '0' and '1' (only), giving them the
   --  meaning 'LOADING' and 'UNLOADING'. for now, we include these in the
   --  type.
   --
   ...
   Move_Func_Directions : constant array
   (Function_T range MOVE_N .. MOVE_NW) of Direction_T :=
     (MOVE_N  => NORTH,
      MOVE_NE => NORTHEAST,
      MOVE_E  => EAST,
      MOVE_SE => SOUTHEAST,
   ...
   Move_Dir_Functions : constant array
   (Direction_T range NORTH .. NORTHWEST) of Function_T :=
     (NORTH     => MOVE_N,
      NORTHEAST => MOVE_NE,
      EAST      => MOVE_E,
      SOUTHEAST => MOVE_SE,
   ...
   --  if we determine how to represent T_PATH, this could be replaced with a
   --  set of constants of type Acceptable_Terrain_Array
   type Terrain_T is (T_UNKNOWN, T_PATH, T_LAND, T_WATER, T_AIR);
   ...
   --  There are 3 maps. 'map' describes the world as it actually exists; it
   --  tells whether each map cell is land, water or a city; it tells whether
   --  or not a square is on the board.
   --
   --  'user_map' describes the user's view of the world. 'comp_map' describes
   --  the computer's view of the world.

   type Real_Map_T is record
   ...
   View : array
   (Piece_Owner_T'Range) of View_Map; --  COMP and USER views of the world

   --  NOTA BENE the above replaced: Comp_Map : View_Map; -- computer's view of
   --  the world User_Map : View_Map; -- user's view of the world

   City : array (1 .. NUM_CITY) of aliased City_Info_T; -- city information

   --  There is one array to hold all allocated objects no matter who owns
   --  them. Objects are allocated from the array and placed on a list
   --  corresponding to the type of object and its owner.
   ...
   Object    : array (1 .. LIST_SIZE) of Piece_Info_P :=
     (others => new Piece_Info_T); -- global object list
   ...
   type Piece_Attr_Array is
     array (Piece_Type_T range ARMY .. SATELLITE) of Piece_Attr_T;
...
private
   ...
   --  values for Piece_Attr.*.Terrain. XXX We could dispatch based on Class
   --  now, but don't yet

   GROUND_TERRAIN : constant Acceptable_Content_Array :=
     ('+' => True, others => False);
   AIRCRAFT_TERRAIN : constant Acceptable_Content_Array :=
     ('.' | '+' => True, others => False);
   SHIP_TERRAIN : constant Acceptable_Content_Array :=
     ('.' => True, others => False);
   ...
   Piece_Attr : constant Piece_Attr_Array :=
     (ARMY =>
        (U_Cont   => 'A',
         C_Cont   => 'a',  -- character for printing piece
         Sel_Char => 'A',
         Class    => GROUND,
   ...
   --  For fighters, the range is set to an even multiple of the speed. This
   --  allows user to move fighter, say, two turns out and two turns back.
      ...
      FIGHTER =>
        (U_Cont      => 'F',
         ...
         Sel_Char    => 'F',
         Class       => AIRCRAFT,
         Name        => Strings.To_Bounded_String ("fighter"),
         Nickname    => Strings.To_Bounded_String ("fighter"),
         Article     => Strings.To_Bounded_String ("a fighter"),
         Plural      => Strings.To_Bounded_String ("fighters"),
      ...
      PATROL =>
        (U_Cont      => 'P',
         ...
         Sel_Char    => 'P',
         Class       => SHIP,
         Name        => Strings.To_Bounded_String ("patrol boat"),
         Nickname    => Strings.To_Bounded_String ("patrol"),
         Article     => Strings.To_Bounded_String ("a patrol boat"),
         Plural      => Strings.To_Bounded_String ("patrol boats"),
      ...
      DESTROYER =>
        (U_Cont      => 'D',
         ...
         Sel_Char    => 'D',
         Class       => SHIP,
         Name        => Strings.To_Bounded_String ("destroyer"),
         Nickname    => Strings.To_Bounded_String ("destroyer"),
         Article     => Strings.To_Bounded_String ("a destroyer"),
         Plural      => Strings.To_Bounded_String ("destroyers"),
         Terrain     => SHIP_TERRAIN,
         Build_Time  => 20,
         Strength    => 1,
         Max_Hits    => 3,
      ...
      SUBMARINE =>
        (U_Cont      => 'S',
         ...
         Sel_Char    => 'S',
         Class       => SHIP,
         Name        => Strings.To_Bounded_String ("submarine"),
         Nickname    => Strings.To_Bounded_String ("submarine"),
         Article     => Strings.To_Bounded_String ("a submarine"),
         Plural      => Strings.To_Bounded_String ("submarines"),
         Terrain     => SHIP_TERRAIN,
         Build_Time  => 20,
         Strength    => 3,
         Max_Hits    => 2,
      ...
      TRANSPORT =>
        (U_Cont      => 'T',
         ...
         Sel_Char    => 'T',
         Class       => SHIP,
         Name        => Strings.To_Bounded_String ("troop transport"),
         Nickname    => Strings.To_Bounded_String ("transport"),
         Article     => Strings.To_Bounded_String ("a troop transport"),
         Plural      => Strings.To_Bounded_String ("troop transports"),
         Terrain     => SHIP_TERRAIN,
         Build_Time  => 30,
         Strength    => 1,
         Max_Hits    => 1,
      ...
      CARRIER =>
        (U_Cont      => 'C',
         ...
         Sel_Char    => 'C',
         Class       => SHIP,
         Name        => Strings.To_Bounded_String ("aircraft carrier"),
         Nickname    => Strings.To_Bounded_String ("carrier"),
         Article     => Strings.To_Bounded_String ("an aircraft carrier"),
         Plural      => Strings.To_Bounded_String ("aircraft carriers"),
         Terrain     => SHIP_TERRAIN,
         Build_Time  => 30,
         Strength    => 1,
         Max_Hits    => 8,
      ...
      BATTLESHIP =>
        (U_Cont      => 'B',
         ...
         Sel_Char    => 'B',
         Class       => SHIP,
         Name        => Strings.To_Bounded_String ("battleship"),
         Nickname    => Strings.To_Bounded_String ("battleship"),
         Article     => Strings.To_Bounded_String ("a battleship"),
         Plural      => Strings.To_Bounded_String ("battleships"),
         Terrain     => SHIP_TERRAIN,
         Build_Time  => 40,
         Strength    => 2,
         Max_Hits    => 10,
      ...
      SATELLITE =>
        (U_Cont      => 'Z',
         ...
         Sel_Char    => 'Z',
         Class       => SPACECRAFT,
         Name        => Strings.To_Bounded_String ("satellite"),
         Nickname    => Strings.To_Bounded_String ("satellite"),
         Article     => Strings.To_Bounded_String ("a satellite"),
         Plural      => Strings.To_Bounded_String ("satellites"),
   ...
   --  we don't rely on all ships being > fighter as the c code does.
   --
   --  NOTA BENE: for user, move order is obeyed WITHIN EACH SECTOR
   --
   --  XXX XXX XXX IT IS CURRENTLY ENTIRELY THE DEVELOPER'S RESPONSIBILITY TO
   --  MAKE SURE EVERY PIECE IS HERE!!!
   Move_Order : constant Move_Order_Array :=
     (SATELLITE,
      TRANSPORT,
      CARRIER,
   ...
   --  lists of attackable objects if object is adjacent to moving piece order
   --  matters! (these are in priority order, most..least)
   ...
   Fighter_Attack : constant Content_Value_Array :=
     ('T'    => 8,
      ...
      others => 0);
   Ship_Attack : constant Content_Value_Array :=
     ('T' => 6, 'C' => 5, 'B' => 4, 'S' => 3, 'D' => 2, 'P' => 1, others => 0);
   ...
   --  attractive, but we are willing to go slightly out of our way to find
   --  a better continent. Similarily for two cities on a continent. At one
   --  city on a continent, things are looking fairly unattractive, and we
   --  are willing to go quite a bit out of our way to find a better continent.
   --
   ...
   --  so we don't need to dump armies on them unless everything else is real
   --  far away. We would prefer to use unloading transports for taking cities
   --  instead of exploring, but we are willing to explore if interesting
   --  cities are too far away.
   --
   --      It has been suggested that continents containing one city
   --  are not interesting. Unfortunately, most of the time what the computer
   --  sees is a single city on a continent next to lots of unexplored
   --  territory. So it dumps an army on the continent to explore the continent
   --  and the army ends up attacking the city anyway. So, we have decided we
   --  might as well send the tt to the city in the first place and increase
   --  the speed with which the computer unloads its tts.
   ...
   Army_Load : constant Move_Info_T :=
     (Owner             => COMP,
      Objective_Weights => ('$' => 1, 'x' => W_TT_BUILD, others => 0));
   ...
   User_Army_Attack : constant Move_Info_T :=
     (Owner             => USER,
      Objective_Weights => ('*' | 'X' | 'a' => 1, ' ' => 12, others => 0));
