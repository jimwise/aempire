...
package body Empire.Objects is
   ...
   function Find_City_At_Loc (Loc    : in Location_T;
                              Owners : in Acceptable_Owner_Array := (others => TRUE))
                             return City_Info_P
   is
   begin
      if Map(Loc).Cityp /= null and then Owners(Map(Loc).Cityp.Owner)
      then
   ...
   begin
      Found := False;                   --  will end up true unless owner has no cities left
      for I in City'Range
      loop
   ...
   function Find_Obj_At_Loc (Loc    : in Location_T;
                             ...
                             Owners :    Acceptable_Owner_Array := (others => TRUE); Ignore_Full : Boolean := False)
                            return Piece_Info_P
   is
   ...
   begin
      ...
      while P /= null
      loop
         if Types(P.Piece_Type) and Owners(P.Owner)
         then
            if Ignore_Full = False or P.Count < Obj_Capacity(P)
            then
               -- if there are multiple acceptable objects at loc, pick the highest value one
               -- XXX XXX `preference' is by location in definition of Piece_Type_T.  This should
               -- XXX XXX change to a tunable piece_value_array
               -- XXX XXX XXX is this actually useful?  Do we actually count on finding
               -- XXX XXX XXX highest value obj at loc?  (probably, but check)

               if Best = null or else P.Piece_Type > Best.Piece_Type
               then
                  Best := P;
               end if;
            end if;
   ...
   begin
      Found := False; --  will end up true unless Owner has no pieces left (matching criteria)
      ...
      loop
         ...
         then
            ...
            then
               -- as with Find_Obj_At_Loc, make sure we pick the `best' object at location
               if Best_Obj = null or else (P.Loc /= Best_Loc or P.Piece_Type > Best_Obj.Piece_Type)
               then
                  Best_Obj := P;
                  Best_Dist := Dist;
                  Best_Loc := P.Loc;
   ...
   -- XXX XXX XXX I've fixed several places where this wasn't being used
   -- XXX XXX XXX (i.e. unmodified base speed was always used).  This needs
   -- XXX XXX XXX to be checked, and ideally made private to force use of
   -- XXX XXX XXX this func.

   function Obj_Moves (Obj : in Piece_Info_P) return Integer
   is
   begin
      return (Piece_Attr(Obj.Piece_Type).Speed * Obj.Hits + Piece_Attr(Obj.Piece_Type).Max_Hits - 1) /
        Piece_Attr(Obj.Piece_Type).Max_Hits;
   end Obj_Moves;

   -- Figure out the capacity for an object
   -- XXX XXX XXX need to check, as with obj_moves, for places where raw capacity is being used, and
   -- XXX XXX XXX fix them, then make the raw capacity private.

   function Obj_Capacity (Obj : in Piece_Info_P) return Integer
   is
   begin
      return (Piece_Attr(Obj.Piece_Type).Capacity * Obj.Hits + Piece_Attr(Obj.Piece_Type).Max_Hits - 1) /
        Piece_Attr(Obj.Piece_Type).Max_Hits;
   end Obj_Capacity;
   ...
   begin
      Disembark(Obj);
      Obj.Hits := 0;
      Obj.Moved := Piece_Attr(Obj.Piece_Type).Speed; --  XXX XXX XXX should not be needed
   ...
   begin
      -- XXX XXX XXX right now, armies at a location are destroyed, while
      -- XXX XXX XXX other hardware is captured.  This works well to discourage
      -- XXX XXX XXX big-ticket building near the front, but may still be too
      -- XXX XXX XXX harsh
      P := Map(City.Loc).objp;
      while P /= null
      loop
   ...
   is
      ...
      Sat_Dir : constant array (Integer range 0 .. 3) of Function_T := (MOVE_NW, MOVE_SW, MOVE_NE, MOVE_SE);
   begin
      -- XXX this can only result in non-zero in the re-tooling case
      -- XXX probably a fairer handling of this case to always revert to
      -- XXX zero after production
      City.Work := City.Work - Piece_Attr(City.Prod).Build_Time;

      -- XXX of course, the static storage model should go at some point
      -- XXX (either make free_list growable, or allocate all dynamically, allowing
      -- XXX us to move away from 'access all' ptrs)
      if Free_List = null
      then
         raise Storage_Error;
      end if;
      ...
      New_Obj.Func := NOFUNC;        --  if city has a func for the obj, this will be handled by caller
      ...
      if New_Obj.Piece_Type = SATELLITE
      then
         -- XXX XXX XXX new satellites get a random direction.  this isn't really ideal -- the user should
   ...
   --      vmap = MAP(obj->owner);
      ...
      Disembark(Obj);                   --  if we were on a ship, we aren't any more

      Lists.Unlink(Map(Old_Loc).Objp, Obj, Loc_Link);
      Lists.Link(Map(New_Loc).Objp, Obj, Loc_Link);

      -- move any objects contained in object
      -- XXX XXX XXX hmmmmmmm.... is it really necessary for contained objects to be linked to map?
      P := Obj.Cargo;
      while P /= null
      loop
         P.Loc := New_Loc;
      ...
      case Obj.Piece_Type is
         when FIGHTER =>
            if Find_City_At_Loc(Obj.Loc) = null
            then
               -- fighters prefer a city to a carrier
               P := Find_Obj_At_Loc(Obj.Loc, (CARRIER => True, others => False), Ignore_Full => True);
               if P /= null
               then
                  Embark(P, Obj);
               end if;
         ...
         when ARMY =>
            ...
            P := Find_Obj_At_Loc(Obj.Loc, (TRANSPORT => True, others => False), Ignore_Full => True);
            if P /= null
            then
               Embark(P, Obj);
            end if;
   ...
   begin
      ...
      loop
         Move_Sat1(Obj);
         if Obj.Piece_Range = 0
         then
            if Obj.Owner = USER
            then
               Ui.Info("Satellite at " & Location_T'Image(Obj.Loc) & " ran out of fuel and burned up on re-entry.");
            end if;
   ...
   function Good_Loc (Obj : in Piece_Info_P;
                      Loc : in Location_T)
                     return Boolean
   is
      C : Content_Display_T;
   ...
   begin
      ...
      then
         P := Find_Obj_At_Loc(Loc, (TRANSPORT => True, others => False), Ignore_Full => True);
         -- XXX easier to check owner here than to pass dynamic acceptable_owner_array above?
         if P /= null and then P.Owner = Obj.Owner
         then
            return True;
         else
      ...
      if Map(Loc).Cityp /= null and then Map(Loc).Cityp.Owner = Obj.Owner
      then
         return True;
      end if;

      if Obj.Piece_Type = FIGHTER
      then
         P := Find_Obj_At_Loc(Loc, (CARRIER => True, others => False), Ignore_Full => True);
         -- XXX easier to check owner here than to pass dynamic acceptable_owner_array above?
         if P /= null and then P.Owner = Obj.Owner
         then
            return True;
   ...
   begin
      ...
      then
         ...
         Dest := Strings.To_Bounded_String(" (" & Location_T'Image(Obj.Dest) & ")");
      else
      ...
      case Obj.Piece_Type is
         when FIGHTER | SATELLITE =>
            Other := Strings.To_Bounded_String("; fuel = " & Integer'Image(Obj.Piece_Range));
         when TRANSPORT =>
            Other := Strings.To_Bounded_String("; armies = " & Integer'Image(Obj.Count));
         when CARRIER =>
            Other := Strings.To_Bounded_String("; fighters = " & Integer'Image(Obj.Count));
         when others =>
            Other := Strings.To_Bounded_String("");
      end case;

      Ui.Prompt(Strings.To_String(Piece_Attr(Obj.Piece_Type).Name) &
               " at " & Location_T'Image(Obj.Loc) &
               ": moves = " & Integer'Image(Obj_Moves(Obj) - Obj.Moved) &
               "; hits = " & Integer'Image(Obj.Hits) &
               "; func = " & Strings.To_String(Function_Name(Obj.Func)) & Strings.To_String(Dest) &
   ...
   begin
      ...
      loop
         case Piece_Attr(Objp.Piece_Type).Class is
            when AIRCRAFT =>
               --  XXX XXX in truth, when we have more than one type of A/C, we'll probably want to differentiate here...
      ...
      Content := Strings.To_Bounded_String(Integer'Image(Aircraft_Count) & " aircraft landed, ");
      ...
      for I in Piece_Type_T'Range
      loop
         if City.Func(I) /= NOFUNC
         then
            Strings.Append(Func, Content_Display_T'Image(Piece_Attr(I).U_Cont));
            Strings.Append(Func, ':');
            Strings.Append(Func, Function_Name(City.Func(I)));
         end if;

         if City.Func(I) = MOVE_TO_DEST
         then
            Strings.Append(Func, " (");
      ...
      Prod := Strings.To_Bounded_String("City at location " & Location_T'Image(City.Loc) &
                                          " will complete " & Strings.To_String(Piece_Attr(City.Prod).Article) &
                                          " on round " & Integer'Image(Date + Piece_Attr(City.Prod).Build_Time - City.Work));
   ...
   begin
      ...
      Update(Owner, Loc);               --  update current location as well (e.g. if we are updating a city we just lost)
   end Scan;
   ...
   begin
      ...
      loop
         ...
         exception
            when Constraint_Error => null; --  2 * dir_offset took us off-offboard
         end;
      end loop;
   ...
   -- Ask the production for a city.  We make sure the city is displayed
   ...
   begin
      ...
      loop
         Ui.Prompt ("What do you want the city at " & Location_T'Image(City.Loc) & " to produce? ");
         T := Ui.Get_Piece_Name;

         if T = NOPIECE
         then
            Ui.Error("I don't know how to build those."); -- XXX kinda snarky.  consider changing.
         else
            City.Prod := T;
            -- retooling time
