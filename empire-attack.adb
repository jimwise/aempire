...
package body Empire.Attack is
   -- Empire.Attack -- handle an attack between two pieces.  We do everything from
   ...
   begin
      ...
      then
         ...
         then
            Ui.Info("The enemy defending the city at " & Location_T'Image(Loc) & " crushed our attacking forces.");
         elsif City_Owner = USER        --  comp attacks, we defend
         then
            Ui.Info("Your city at " & Location_T'Image(Loc) & " is under attack.");
   ...
   begin
      ...
      if Piece_Attr(Def_Obj.Piece_Type).class = SPACECRAFT
      then
      ...
      loop
         ...
         then
           Att_Obj.Hits := Att_Obj.Hits - Piece_Attr(Def_Obj.Piece_Type).Strength;
         else
            Def_Obj.Hits := Def_Obj.Hits - Piece_Attr(Att_Obj.Piece_Type).Strength;
         end if;
      end loop;
      ...
      then
         ...
         then
            case Win_Obj.Cargo.Piece_Type is
               when ARMY =>
                  Ui.Info(Integer'Image(Diff) & " armies were lost on the victorious transports.");
               when FIGHTER =>
                  Ui.Info(Integer'Image(Diff) & " fighters were lost on the victorious carriers.");
               when others =>
                  raise Program_Error;
            end case;
         end if;
