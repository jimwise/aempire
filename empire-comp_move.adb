...
package body Empire.Comp_Move is
   ...
   begin
      ...
      for I in Piece_Type_T'Range loop
         ...
         while Obj /= null loop
            Ui.Prompt
              ("ENEMY: UPDATING PIECE AT " & Location_T'Image (Obj.Loc));
...
--  Handle city production. First, we set production for new cities. Then
--  we produce new pieces. After producing a piece, we will see if we should
--  change our production.
--
--  Our goals for city production are first, not to change production while
--  something is in the process of being built. Second, we attempt to have
--  enough city producing armies on a continent to counter any threat on the
--  continent, and to adequately explore and control the continent. Third, we
--  attempt to always have at least one transport producer. Fourth, we attempt
--  to maintain a good ratio between the number of producers we have of each
--  type of piece. Fifth, we never build carriers, as we don't have a good
--  strategy for moving these.
--
--  XXX user_move can now use carriers to refuel planes, once we add XXX same
--  code here, we just need a strategy to position and guard XXX carriers

   procedure Do_Cities is
      Is_Land_Locked : Boolean;
   begin
      for I in City'Range loop
         if City (I).Owner = COMP then
            Objects.Scan (COMP, City (I).Loc);

            if City (I).Prod = NOPIECE then
      ...
      loop
         if City (I).Owner = COMP then
            ...
            City (I).work  :=
              City (I).work +
              1; -- was post-increment in c, but this was wrong (?)
...
--  3) Meet the ratio requirements given above.
   ...
   is
      City_Count : Piece_Value_Array :=
        (others => 0); -- # of cities producing each piece
   ...
   begin
      ...
      for I in View (COMP)'Range loop
         if Cont_Map (I)                 -- for each cell of continent
         then
            P :=
              Objects.Find_City_At_Loc
                (I,
                 Owners => (COMP => True, others => False));
            if (P /= null) and then (P.Prod = ARMY) then
      ...
      Interest :=
        (Counts.Unexplored > 0) or
        (Counts.User_Cities > 0) or
        (Counts.User_Objects (ARMY) > 0) or
        (Counts.Unowned_Cities > 0);

      --  we want one more army producer than enemy has cities and one more if
      --  anything of interest on continent
      ...
      if (Counts.User_Cities > 0) and (Cityp.Prod = NOPIECE) then
         Comp_Set_Prod (Cityp, ARMY);
         return;
      end if;
      ...
      for I in City'Range loop
         if (City (I).Owner = COMP) and (City (I).Prod /= NOPIECE) then
            City_Count (City (I).Prod) := City_Count (City (I).Prod) + 1;
            Total_Cities               := Total_Cities + 1;
         end if;
      end loop;

      if Total_Cities <= 10 then
         Ratio := 1;
      elsif Total_Cities <= 20 then
         Ratio := 2;
      elsif Total_Cities <= 30 then
         Ratio := 3;
      else
         Ratio := 4;
      end if;
      ...
      if (City_Count (ARMY) = 1) and (Cityp.Prod = ARMY) then
         return;
      end if;

      --  first available non-land-locked city becomes a tt producer
      if City_Count (TRANSPORT) = 0 then
         if not Is_Land_Locked then
            Comp_Set_Prod (Cityp, TRANSPORT);
            return;
         end if;

         --  if we have one army producer that is not on a land-locked, produce
         --  armies here instead
         if City_Count (ARMY) = 1 then
            for I in City'Range loop
               if (City (I).Owner = COMP) and (City (I).Prod = ARMY) then
                  if not Land_Locked (City (I).Loc) then
                     Comp_Set_Prod (Cityp, ARMY);
                     return;
                  end if;
      ...
      if (Cityp.Prod = ARMY) and Interest then
         return;
      end if;

      --  Produce armies in new cities if there is a city to attack
      if (Counts.Unowned_Cities > 0) and Cityp.Prod = NOPIECE then
         Comp_Set_Prod (Cityp, ARMY);
         return;
      end if;

      --  Set production to item most needed. Continents with one city and
      --  nothing interesting may not produce armies. We set production for
      --  unset cities, and change production for cities that produce objects
      --  for which we have many city producers. Ship producers on lakes also
      --  get there production changed.
      ...
      case Cityp.Prod is
         ...
         when others =>
            if Overproduced (Cityp, City_Count) then
               Comp_Set_Needed (Cityp, City_Count, Interest, Is_Land_Locked);
            end if;
      end case;
...
--  Set production for a computer city to a given type. Don't reset production
...
--  See if a city is producing an object which is being overproduced
   ...
   begin
      for I in Piece_Type_T'Range loop
         ...
         if (Cityp.Prod /= I) and
           (((City_Count (Cityp.Prod) - 1) * Ratios (Ratio) (I)) >
            ((City_Count (I) + 1) * Ratios (Ratio) (I)))
         then
            return True;
         end if;
      end loop;
...
--  See if one type of production is needed more than another type. Return the
--  most-needed type of production.
   ...
   is
   begin
      if (City_Count (Prod1) * Ratios (Ratio) (Prod2)) <=
        (City_Count (Prod2) * Ratios (Ratio) (Prod1))
      then
         return Prod1;
      else
         return Prod2;
...
--  Figure out the most needed type of production. We are passed a flag telling
--  us if armies are ok to produce.
   ...
   begin
      ...
      for Prod in Piece_Type_T'Range loop
...
--  See if a city is land-locked (not next to water or only next to a lake).
--  We define a lake to be a body of water (where cities are considered to
--  be water) that does not touch either an attackable city or unexplored
--  territory.

--  Be careful, because we use the 'emap'. This predicts whether unexplored
--  territory will be land or water. The prediction should be helpful, because
--  small bodies of water that enclose unexplored territory will appear as
--  solid water. Big bodies of water should have unexplored territory on the
--  edges.
   ...
   begin
      ...
      return (Counts.Unowned_Cities > 0) or
        (Counts.User_Cities > 0) or
        (Counts.Unexplored > 0);
   end Land_Locked;
   ...
   begin
      for I in Move_Order'Range loop
         ...
         while Obj /= null loop
            Next_Obj :=
              Obj.Links (Piece_Link).Next; -- set now, in case obj is destroyed
...
--  Move a piece. We loop until all the moves of a piece are made. Within the
--  loop, we find a direction to move that will take us closer to an objective.
   ...
   begin
      ...
      if Obj.Piece_Type = SATELLITE then
      ...
      if Piece_Attr (Obj.Piece_Type).Piece_Range /= INFINITY then
      ...
      while Obj.Moved < Objects.Obj_Moves (Obj) loop
         ...
         then
            if View (COMP) (Obj.Loc).Contents = 'X' then
               --  remember, we've already move1'ed above. so if we ended in a
               --  city, just end
...
--     destination.  (If there is no destination, sit around and wait.)
   ...
   begin
      ...
      then
         Load_Army
           (Obj);                -- make sure we're on the best ship present
...
--  old code checked obj.ship for null, but we would have excepted in that case
      ...
      then
         ...
         --  otherwise, sacrifice ourselves to kill nearby ships/planes if
         --  needed
         New_Loc :=
           Find_Attack
             (Obj.Loc,
      ...
      then
         ...
         if View (COMP) (New_Loc).Contents = '.' and Obj.Hits > 0 then
            ...
            Objects.Scan
              (USER,
               New_Loc);    -- rescan for user, since army is gone
         end if;
      ...
      then
         if Obj.Ship.Func = COMP_LOADING then
            Load_Army
              (Obj);             -- original code paniced on fail, we except
            return;                     -- armies stay on a loading ship
         end if;

         Make_Unload_Map (Amap, View (COMP));
         Mapping.Vmap_Find_Sealand_Obj
           (New_Loc,
            Pmap,
            Amap,
            Obj.Loc,
      ...
      if New_Loc /= Obj.Loc then
         ...
         case View (COMP) (New_Loc).Contents is
            --  by setting cross_cost, we argue against going elsewhere on the
            --  map
            when 'A' | 'O' =>
      ...
      if Cross_Cost > 0 then
         ...
         then
            --  Load an army onto a ship. First look for an adjacent ship. If
            --  that doesn't work, move to the objective, trying to be close to
            --  the ocean. Note: this was a function in the original, but was
            --  only called here.
            begin
               Load_Army (Obj);
            exception
...
--  Remove pruned explore locs from a view map
   ...
   begin
      for I in Xmap'Range loop
         if Map (I).On_Board and (Xmap (I).Contents = ' ') then
            Xmap (I).Contents := Emap (I).Contents;
         end if;
      end loop;
   end Unmark_Explore_Locs;

--  Make a load map. We copy the view map and mark each loading transport and
--  transport-producing city with a '$'.
   ...
   begin
      ...
      for I in City'Range loop
         if (City (I).Owner = COMP) and (City (I).Prod = TRANSPORT) then
            if Nearby_Load (Obj, City (I).Loc) then
               xmap (City (I).Loc).Contents :=
                 'x'; -- army is nearby so it can load
            elsif Nearby_Count (City (I).Loc) <
              Piece_Attr (TRANSPORT).Capacity
            then
...
--  Return true if an army is considered near a location for loading
   ...
   is
   begin
      return (Obj.Func = COMP_LOADING) and
        (Math.Dist (Obj.Loc, Loc) <= LOAD_RADIUS);
   end Nearby_Load;
...
--  Return number of nearby armies. */
   ...
   begin
      ...
      if Print_Vmap = 'L' then
         Ui.Print_Zoom (Xmap);
      end if;
   end Make_Tt_Load_Map;

--  Make an unload map. We copy the view map. We then create a continent map.
--  For each of our cities, we mark out the continent that city is on. Then,
--  for each city that we don't own and which doesn't appear on our continent
--  map, we set that square to a digit.
--
--  We want to assign weights to each attackable city. Cities are more valuable
--  if they are on a continent which has lots of cities. Cities are also
--  valuable if either it will be easy for us to take over the continent, or
--  if we need to defend that continent from an enemy.
--
...
--  d) Any other attackable city is marked with a '0'.
   ...
   begin
      ...
      for I in Vmap'Range loop
         if (Vmap (I).Contents = 'O') or (Vmap (I).Contents = '*') then
            ...
            if (Counts.User_Cities > 0) and (Counts.Comp_Cities > 0) then
               Xmap (I).Contents := Comp_Map_Weight (Total_Cities);
            elsif (Counts.Unowned_Cities > Counts.User_Cities) and
              (Counts.Comp_Cities = 0)
            then
               Xmap (I).Contents := Comp_Map_Weight (Total_Cities);
            elsif (Counts.User_Cities = 1) and (Counts.Comp_Cities = 0) then
   ...
   begin
      case Weight is
         ...
         when 9 =>
            return '9';
         when others =>
            --  perhaps this should change, but this is what the old code did,
            --  in effect
            return '0';
      end case;
   end Comp_Map_Weight;

--  Look for the most full, non-full transport at a location. prefer switching
--  to staying - if we switch, we force one of the ships to become more full.
   ...
   begin
      P := Map (Loc).ObjP;
      while P /= null loop
         if (P.Piece_Type = TRANSPORT) and
           (Objects.Obj_Capacity (P) > P.Count)
         then
            if (Best = null) or else (P.Count >= Best.Count) then
               Best := P;
...
--  Return the first location we find adjacent to the current location of the
--  correct terrain.
   ...
   begin
      for I in Dir_Offset'Range loop
         ...
         if Map (New_Loc).On_Board and Terrain (Vmap (New_Loc).Contents) then
            return New_Loc;
         end if;
      end loop;
...
--  Look to see if there is an adjacent object to attack. We are passed a
--  location and a list of items we attack sorted in order of most valuable
--  first. We look at each surrounding on board location. If there is an
--  object we can attack, we return the location of the best of these.
   ...
   begin
      for I in Dir_Offset'Range loop
         ...
         if Map (New_Loc).On_Board and Terrain (Map (New_Loc).Contents) then
            P := Obj_List (View (COMP) (New_Loc).Contents);
            if (P /= 0) and (P > Best_Val) then
               Best_Val := P;
               Best_Loc := New_Loc;
            end if;
         end if;
...
--
--  Loading transports move toward loading armies. Unloading transports move
--  toward attackable cities on unowned continents.
--
...
--  Transports become 'loading' when empty, and 'unloading' when full.
   ...
   begin
      ...
      then
         ...
         New_Loc  :=
           Find_Attack (Obj.Loc, Tt_Attack, ('.' => True, others => False));
...
--  return to base if low on fuel XXX XXX XXX should check for carriers --
--  compare user_move, which now does.
      Objects.Find_Nearest_City
        (Obj.Loc,
         ...
         New_Loc,
         City_Cost);
      if City_Found and Obj.Piece_Range <= City_Cost + 2 then
         if New_Loc /= Obj.Loc then
...
--  something to attack.
--
--  XXX carriers should probably have their own routine -- they shouldn't be
--  attacking XXX things, and they should be making themselves more useful
   ...
   begin
      ...
      if Obj.Hits < Piece_Attr (Obj.Piece_Type).Max_Hits then
         ...
         New_Loc :=
           Find_Attack (Obj.Loc, Ship_Attack, ('.' => True, others => False));
         if New_Loc /= Obj.Loc          -- something to attack?
         then
            Attack.Attack (Obj, New_Loc);
            return;
   ...
   is
      ...
      D           : Integer;
      --  XXX XXX not clear if map reuse saves us much. To find out, set this
      --  to FALSE, and compare XXX XXX profiling output.
      Reuse : Boolean :=
        True;          -- try to reuse unless we learn otherwise
      Old_Loc  : Location_T;
      Old_Dest : Location_T;
      Pmap     : Mapping.Path_Map;
   ...
   begin
      ...
      if New_Loc = Obj.Loc and          -- path is blocked?
        (Piece_Attr (Obj.Piece_Type).Class /= GROUND) and
        (Obj.Ship /= null) -- don't unblock armies on a ship
      then
      ...
      if (New_Loc = Obj.Loc) and
        (Map (Obj.Loc).CityP /= null) and
        (Piece_Attr (Obj.Piece_Type).Class = GROUND)
      then
         New_Loc :=
           Move_Away (View (COMP), Obj.Loc, ('+' => True, others => False));
         Reuse := False;
      end if;
      ...
      --  now, if we have not made the Path_Map un-reusable, immediately try
      --  to chain a second move onto the first, to save us an iteration.
      --  It is _not_ clear how much this saves at this point, so given its
      --  complexity, we should profile and (perhaps) eliminate this. Way to
      --  check would be to set default value of `Reuse' (above) to `FALSE'
      --  and see how profile data changes
      --
      --  Note also that this is error prone in that behavior of a chained move
      --  may not exactly match what we would have done if not chaining.

--  if we can reuse the path map, and we have more moves, and we haven't
--  reached our objective yet
      ...
      then
         ...
         case Obj.Piece_Type is
            ...
            when ARMY =>
               ...
               --  IMPORTANT (XXX): in the original code, these had 'X' instead
               --  of '*', which could _never_ match (since Find_Attack uses
               --  Map, not View(COMP)) (!). This means that in the original
               --  code, each of these acted as if the city was _not_ included.
               --  THIS IS A BEHAVIOR CHANGE! (and skill of Ai should be tested
               --  accordingly!)
            ...
            when TRANSPORT =>
               Terrain := ('.' | '*' => True, others => False);
               if Obj.Cargo /= null then
                  Attack_List := Tt_Attack;
               else
                  Attack_List :=
                    ('*' => 2, 'O' => 1, others => 0); -- cause TT to wake up
               end if;
         ...
         if Find_Attack (Obj.Loc, Attack_List, Terrain) /= Obj.Loc then
            return;
         end if;

         --  clear old path before moving, so we don't oscillate
         Pathmap.Clear (Old_Loc);
         --  Pathmap(Old_Loc).Terrain := T_UNKNOWN; for I in Dir_Offset'Range
         --  loop
   ...
   is
      Water_Chars : constant Acceptable_Content_Array :=
        ('.' | '*' => True, others => False);
   begin
   ...
   function Vmap_Land
     (Vmap : in View_Map;
      Loc  : in Location_T) return Continent_Map
   is
      Land_Chars : constant Acceptable_Content_Array :=
        ('+' | '*' => True, others => False);
   begin
   ...
   is
      Cont_Map : Continent_Map                 := (others => False);
      Seen     : array (Location_T) of Boolean :=
        (others => False); --  cells we've examined
      Workset      : Location_Vector;
      Cur, New_Loc : Location_T;
   begin
      --  test if loc is acceptable, or raise
      if not Good_Terrain (Map (Loc).Contents) then
         raise Program_Error;
      end if;
      ...
      while not Workset.Is_Empty loop
         ...
         for D in Direction_T'Range loop
            ...
            --  if unexplored (per vmap) or desired_terrain (per vmap) or
            --  desired terrain (per map) we check desired terrain in two
            --  places, so we can use the pruned Emap if desired
            if Map (New_Loc).On_Board and not Seen (New_Loc) then
               if Vmap (New_Loc).Contents = ' '
                 or else Good_Terrain (Vmap (New_Loc).Contents)
                 or else Good_Terrain (Map (New_Loc).Contents)
               then
   ...
   --  Scan a continent recording items of interest on the continent. XXX XXX
   --  XXX This could be done as we flood-fill the continent.
      ...
      for I in Location_T'Range loop
         if Cont_Map (I) then
            ...
            case Vmap (I).Contents is
               ...
               when 'F' =>
                  Incr (Counts.User_Objects (FIGHTER));
               when 'P' =>
                  Incr (Counts.User_Objects (PATROL));
               when 'D' =>
                  Incr (Counts.User_Objects (DESTROYER));
               when 'S' =>
                  Incr (Counts.User_Objects (SUBMARINE));
               when 'T' =>
                  Incr (Counts.User_Objects (TRANSPORT));
               when 'C' =>
                  Incr (Counts.User_Objects (CARRIER));
               when 'B' =>
                  Incr (Counts.User_Objects (BATTLESHIP));

               when 'X' =>
                  Incr (Counts.Comp_Cities);

               when 'a' =>
                  Incr (Counts.Comp_Objects (ARMY));
               when 'f' =>
                  Incr (Counts.Comp_Objects (FIGHTER));
               when 'p' =>
                  Incr (Counts.Comp_Objects (PATROL));
               when 'd' =>
                  Incr (Counts.Comp_Objects (DESTROYER));
               when 's' =>
                  Incr (Counts.Comp_Objects (SUBMARINE));
               when 't' =>
                  Incr (Counts.Comp_Objects (TRANSPORT));
               when 'c' =>
                  Incr (Counts.Comp_Objects (CARRIER));
               when 'b' =>
                  Incr (Counts.Comp_Objects (BATTLESHIP));
               ...
               when 'Z' |
                 'z' =>                --  check for city underneath satellite
   ...
   --  Prune unexplored territory. We take a view map and we modify it so
   --  that unexplored territory that is adjacent to a lot of land or a lot
   --  of water is marked as being either that land or water. So basically,
   --  we are making a predicition about what we expect for land and water. We
   --  iterate this algorithm until either the next iteration would remove all
   --  unexplored territory, or there is nothing more about which we can make
   --  an assumption.
   --
   --  First, we use an explore_map to save the number of adjacent land and
   --  water cells for each unexplored cell. Cells which have adjacent explored
   --  territory are placed in a perimeter list. We also count the number of
   --  cells that are not unexplored.
   --
   --  We now take this perimeter list and make high-probability predictions.
   --
   --  Then we round things off by making one pass of medium probability
   --  predictions.
   --
   --  Then we make multiple passes extending our predictions.
   --
   --  We stop if at any point all remaining unexplored cells are in a
   --  perimeter list, or if no predictions were made during one of the
   --  final passes.
   --
   --  Unlike other algorithms, here we deal with "off board" locations. So
   --  be careful. XXX XXX XXX is this necessary?! would be cleaner to code
   --  without this caveat...

   procedure Vmap_Prune_Explore_Locs (Vmap : in out View_Map) is
   ...
   begin
      ...
      for L in Location_T'Range loop
         if Vmap (L).Contents /= ' ' then
            Explored := Explored + 1;
            ...
            for D in Direction_T'Range loop
               begin
                  ...
                  case Vmap (New_Loc).Contents is
                     when ' ' =>
                        null;  --  ignore adjacent unexplored
                     when '.' =>
                        Exmap (L).Water := Exmap (L).Water + 1; --  count water
                     --  XXX does this count ships as land?
                     when others =>
                        Exmap (L).Land := Exmap (L).Land + 1; --  count_land
                  end case;
               exception
      ...
      loop    --  do high-probability predictions
         if (From.Last_Index) + Explored = MAP_SIZE then
         ...
         for I in 1 .. From.Last_Index loop
            Loc := From.Element (I);
            if Exmap (Loc).Land >= 5 then
               Expand_Prune (Vmap, Exmap, Loc, T_LAND, To, Explored);
            elsif Exmap (Loc).Water >= 5 then
            ...
            then
               Expand_Prune (Vmap, Exmap, Loc, T_WATER, To, Explored);
            elsif (Loc = 0 or Loc = MAP_SIZE - 1) and
              Exmap (Loc).Land >= 2
            then
               Expand_Prune (Vmap, Exmap, Loc, T_LAND, To, Explored);
            elsif (Loc = 0 or Loc = MAP_SIZE - 1) and
      ...
      for I in 1 .. From.Last_Index loop
         Loc := From.Element (I);
         if Exmap (Loc).Land > Exmap (Loc).Water then
            Expand_Prune (Vmap, Exmap, Loc, T_LAND, To, Explored);
         elsif Exmap (Loc).Land < Exmap (Loc).Water then
            Expand_Prune (Vmap, Exmap, Loc, T_WATER, To, Explored);
         else         --  copy perimeter cell as is
            To.Append (Loc);
            --  note we don't track `Copied' here, as it is not used in this
            --  loop (it will be zero'ed in next loop)
         end if;
      end loop;
      ...
      loop
         ...
         for I in 1 .. From.Last_Index loop
            Loc := From.Element (I);
            if Exmap (Loc).Land >= 4 and Exmap (Loc).Water < 4 then
               Expand_Prune (Vmap, Exmap, Loc, T_LAND, To, Explored);
            elsif Exmap (Loc).Water >= 4 and Exmap (Loc).Land < 4 then
               Expand_Prune (Vmap, Exmap, Loc, T_WATER, To, Explored);
            elsif (Loc < MAP_WIDTH or Loc >= MAP_SIZE - MAP_WIDTH) and
   ...
   --  Expand an unexplored cell. We increment the land or water count of each
   --  neighbor. Any neighbor that acquires a non-zero count is added to the
   --  'to' perimiter list. The count of explored territory is incremented.
   --
   ...
   begin
      ...
      for D in Direction_T'Range loop
         begin
            ...
            if Vmap (New_Loc).Contents = ' ' then
               if Exmap (New_Loc).Land = 0 and Exmap (New_Loc).Water = 0 then
                  To.Append (New_Loc);
               end if;
               if Ttype = T_LAND then
