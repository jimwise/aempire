...
--  supports XXX XXX XXX a path. they should be merged.
   ...
   begin
      ...
      then
         --  XXX XXX XXX it would be cleaner to do this elsewhere -- moved
         --  status is not otherwise updated here...
         Obj.Moved := Objects.Obj_Moves (Obj);
         Awoken    := False;
         return;
      end if;

      if Obj.Func = NOFUNC then
         Awoken := True;
         return;                        --  object was already awake
      end if;

      if Piece_Attr (Obj.Piece_Type).Class = AIRCRAFT and Obj.Func /= LAND then
         --  original didn't consider carriers, and didn't consider aircraft
         --  with a destination. XXX this still isn't quite right -- a carrier
         --  might have room now, and be XXX in range now, but not by the time
         --  we get there. We try to compensate XXX for this by biasing against
         --  carriers, but the right solution, IMO, is XXX to have an AC which
         --  is manually landed on a carrier `bind' to that carrier XXX, and
         --  always take range ONLY to that specific carrier until manually
         --  landed XXX elsewhere
         declare
            City_Found    : Boolean;
         ...
         begin
            ...
            Objects.Find_Nearest_Obj
              (Obj.Loc,
               ...
               Carrier_Range);
            if Carrier_Found then
               if not City_Found or Carrier_Range < Refuel_Range then
                  Refuel_Range := Carrier_Range;
               end if;
            end if;
            if Obj.Piece_Range <= Refuel_Range + 2 then
               User_Obj_Func (Obj, NOFUNC);
               Awoken := True;
               return;
...
--  the wrong direction, and be stranded
      ...
      for I in Direction_T'Range loop
         ...
         if C = '*' or Comp_Content (C) then
            if Obj.Func /= MOVE_TO_DEST then
               Ui.Debug_Error
                 ("awakening unit at " &
                  Location_T'Image (Obj.Loc) &
                  " due to adjacent unit or city");
               User_Obj_Func (Obj, NOFUNC);
               Awoken := True;
               return;
            end if;
