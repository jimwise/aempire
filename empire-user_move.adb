...
package body Empire.User_Move is
   ...
   procedure Piece_Move (Obj : Piece_Info_P) is
      Changed_Loc : Boolean := false;
      Speed : Integer := Piece_Attr(Obj.type).speed;
      Max_Hits : Integer = Piece_Attr(Obj.type).Max_Hits;
      ...
      if Cityp /= null and then Cityp.Func(Obj.type) /= NOFUNC
      then
      ...
      loop
         ...
         -- if Obj takes damage
         exit when Obj.Moved >= Objects.Obj_Moves(Obj);
