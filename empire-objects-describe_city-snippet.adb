package Empire.Editing is
   ...
   -- XXX XXX if called with Func = MOVE_TO_DEST and no Dest_If_Move_To_Dest argument, sets piece/pieces made by city to move to destination 0.
   procedure E_User_Obj_Func (Loc : in Location_T; Func : in Function_T;  Ptypes : in Acceptable_Piece_Array := (SATELLITE => FALSE, others => TRUE); Dest_If_Move_To_Dest : Location_T := 0);
   procedure E_User_City_Func (Loc : in Location_T; Func : in Function_T; Ptype : in Piece_Type_T; Dest_If_Move_To_Dest : Location_T := 0);
...
private
   ...
   -- information on a path being constructed.  No path end, as end is edit location at time path is ended.
   -- XXX XXX this should grow waypoint support
   type Path_T is
      record
