package Empire.Objects is
   ...
   --  originals returned Loc as found loc to indicate none found; XXX maybe
   --  still should? check users... XXX XXX XXX these two should have the
   --  same signature, and probably the latter two should be one general
   --  Find_Nearest_Obj XXX XXX XXX with filters like Find_Obj_At_Loc...

   --  XXX XXX XXX IMPORTANT: these use the map, not a vmap, and as such
   --  should only be called with Owner set to the calling XXX XXX XXX
   --  player. To encourage this, Owner is an Owner_T, unlike the versions
   --  in Find_*_At_Loc, and is non-optional

   --  XXX XXX re-order parameters to allow three filter args to come last.
   --  then re-order other Find_* to match
   ...
   --  this belongs elsewhere, since it queries the user (and doesn't otherwise
   --  search)
   function Good_Loc
     (Obj : in Piece_Info_P;
   ...
   procedure Scan (Owner : in Piece_Owner_T; Loc : in Location_T);
...
private
   ...
   procedure Scan_Sat (Owner : in Piece_Owner_T; Loc : in Location_T);
   procedure Update (Owner : in Piece_Owner_T; Loc : in Location_T);
end Empire.Objects;
